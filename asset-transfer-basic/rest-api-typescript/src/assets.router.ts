...
assetsRouter.get('/search/:owner', async (req: Request, res: Response) => {
  const owner = req.params.owner;
  logger.debug('Search assets by owner request received for owner %s', owner);

  try {
    const mspId = req.user as string;
    const contract = req.app.locals[mspId]?.assetContract as Contract;

    const data = await evatuateTransaction(contract, 'SearchByOwner', owner);
    let assets = [];
    if (data.length > 0) {
      assets = JSON.parse(data.toString());
    }
    ...
    logger.error(
      { err },
      'Error processing search assets by owner request for owner %s',
      owner
    );
    return res.status(INTERNAL_SERVER_ERROR).json({
      status: getReasonPhrase(INTERNAL_SERVER_ERROR),
...
assetsRouter.post(
  '/search',
  body().isObject().withMessage('body must contains key and value'),
  body('key', 'must be a string').isString(),
  body('value', 'must be a string or a JSON string').isString(),
  async (req: Request, res: Response) => {
    logger.debug('Search by Key:Value request received');
    ...
    try {
      ...
      logger.error({ err }, 'Error processing search assets request');
      return res.status(INTERNAL_SERVER_ERROR).json({
        status: getReasonPhrase(INTERNAL_SERVER_ERROR),
        timestamp: new Date().toISOString(),
      });
...
assetsRouter.post(
  ...
  body('data', 'must be a JSON string').isString(),
  async (req: Request, res: Response) => {
